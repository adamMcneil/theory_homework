...
int countIntersection(struct LineSegments* lineSegments) {
    ...
    for (int i = 0; i < points->numberOfPoints; i++) {
        ...
        if (currentPoint->isStart) {
            ...
            // printLabels(labels, length);
            // printf("%d\n", currentIndex);
            if (currentIndex-1 > -1 && intersect(lineSegments->lineSegments[labels[currentIndex]->index], lineSegments->lineSegments[labels[currentIndex-1]->index])) {
                count++;
            }
            if (currentIndex+1 < length && intersect(lineSegments->lineSegments[labels[currentIndex]->index], lineSegments->lineSegments[labels[currentIndex+1]->index])) {
                count++;
            }
        }
        else {
            ...
            length--;
            // printLabels(labels, length);
            if (currentIndex-1 > -1 && currentIndex < length && intersect(lineSegments->lineSegments[labels[currentIndex-1]->index], lineSegments->lineSegments[labels[currentIndex]->index])){
                count++;
            }
        }
    }
